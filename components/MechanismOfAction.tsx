...
export default function MechanismOfAction({ interventions }: Props) {
  ...
  // Filter to real drug interventions only — skip placebos, class descriptions etc
  ...
  async function loadAllDrugs() {
    ...
    // Auto-open first result with useful MOA data
    const firstFound = allResults.findIndex(
      (r) => r.found && r.info?.mechanismOfAction
    );
    if (firstFound !== -1) setExpanded(firstFound);
    else {
      const firstAny = allResults.findIndex((r) => r.found);
      if (firstAny !== -1) setExpanded(firstAny);
    }
  }
  ...
  return (
    <section className="border border-gray-200 rounded-xl bg-white p-6">
      ...
      <p className="text-sm text-gray-500 mb-4">
        Sourced from FDA, ChEMBL, PubChem and published literature
      </p>
            ...
            >
              ...
              >
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {(result.info as any)?.confidence && (
                    <ConfidenceBadge
                      confidence={(result.info as any).confidence}
                    />
                  )}
                   ...
                   result.info.source !== "wikipedia" && (
                    <div className="px-4 pb-4 space-y-4 bg-gray-50">
                      ...
                      {result.info.source === "ai" && (
                        <div className="pt-2 px-3 pb-3 bg-violet-50
                                        border border-violet-100 rounded-lg
                                        text-xs text-violet-700
                                        flex items-start gap-2">
                          <span className="shrink-0">⚠️</span>
                          <span>
                            Generated from published literature, patents and
                            clinical trial records. Verify with primary sources
                            before clinical use.
                          </span>
                        </div>
                      )}
                   ...
                   result.info.source === "wikipedia" && (
                    <div className="px-4 py-4 bg-amber-50 space-y-3">
                      ...
                      <p className="text-xs text-amber-700">
                        Not found in FDA, ChEMBL or PubChem — likely an
                  ...
                  {!result.found && !result.error && (
                    <div className="px-4 py-4 bg-yellow-50">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <a
                          href={`https://www.google.com/search?q=${encodeURIComponent(result.name + " mechanism of action clinical trial")}`}
                          target="_blank" rel="noopener noreferrer"
                          className="text-xs px-3 py-1.5 bg-white border
                                     border-yellow-300 rounded-lg text-yellow-800
                                     hover:bg-yellow-100 transition-colors"
      ...
      {!loading && results.length > 0 && (
        <p className="text-xs text-gray-400 mt-4 pt-4
          ...
          Data from FDA openFDA, EMBL-EBI ChEMBL, NIH PubChem,
          published literature and AI synthesis.
        </p>
      )}
